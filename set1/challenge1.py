...
def get_bit_in_byte(b, index, offset=False):
    if index > 7:
        raise IndexError
...
def get_bit_in_byte_array(byte_array, index):
    ...
    bit = get_bit_in_byte(target_byte, bit_index_in_byte, offset=True)
    return bit
...
def main():
    ...
    final_result = "SSdtIGtpbGxpbmcgeW91ciBicmFpbiBsaWtlIGEgcG9pc29ub3VzIG11c2hyb29t"
    if not test_base64(bytes.fromhex(hex_string), final_result):
